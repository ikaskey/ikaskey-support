...
ユーザーからの追加依頼は、専用の申請サイト **[ブキチの絵文字工場](https://ikaskey-emoji.bktsk.com/)** から申請してください。いかすきー本体のアカウントでログイン (MiAuth) し、フォームに必要事項を入力する形式です。  
なお、利用用途によって悪意をもって利用される可能性の高いものは、管理者またはモデレーターにより申請が却下される、または修正を求められる可能性があります。

:::tip[申請サイトの 2 つのモード]
- **アップロード**: 画像ファイルを直接アップロードして申請
- **他鯖から取り込み**: 他の Misskey サーバーにある絵文字を host + name 指定で取り込み申請

申請時にすでに同じ name の絵文字がいかすきー本体に存在する場合は、フォーム送信時にエラーとなり別名への修正を求められます。
:::
...
- 絵文字の名前（:で囲まれる文字列）は、フォームの「絵文字名」欄に半角英数字とアンダースコア (`_`) で入力してください。
  - 日本語など全角文字は不可
  - 原則としてヘボン式の表記とする（ 参考: [ヘボン式ローマ字綴方表](https://www.ezairyu.mofa.go.jp/passport/hebon/) ）
...
上記のチェックを経てモデレーターが申請を「採用」すると順次登録されていきます。  
また、状況によりローカルのみ・センシティブ設定とする場合があります。（本人からの設定してほしいとする申告がある場合にはそれを加味して判断します。）

登録後はスーパーマンタローから採用 / 却下のお知らせ note がメンションで届きますので、それまでお待ちください。（申請から 1 週間程度かかる場合があります）
...
他サーバーからの絵文字インポートも、同じく [ブキチの絵文字工場](https://ikaskey-emoji.bktsk.com/) の「**他鯖から取り込み**」タブから申請を行ってください。手順は以下のとおりです。

1. 該当のサーバーからいかすきーのユーザー (自分など、誰でも可) に対して該当の絵文字を利用した Note をメンションで送信する
   - これにより、いかすきー側にその絵文字データがキャッシュされます (Misskey の仕様)
2. いかすきー内で該当の絵文字が表示されていることを確認する
3. ブキチの絵文字工場の「他鯖から取り込み」タブで、インポート元のサーバー host (例: `misskey.io`) + 絵文字 name を入力 → 検索 → プレビューを確認 → 申請

事前に該当サーバーからの送信がされていないとデータが取得できずインポートができません。申請前に該当の絵文字を自分などへメンション送信（いかすきーが受信していれば良いので、ダイレクトでも可）してから申請をお願いします。

:::caution[Misskey アップデートに伴う補足]
一部のサーバーは外部サーバーへの絵文字インポートを拒否していることがあります。  
その際には画像をダウンロードし、ブキチの絵文字工場の「アップロード」タブから通常と同様に申請を行ってください。
:::
